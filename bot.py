...
EXCLUDE_LISTS = {
    "exclude 1": {"link": "", "note": "EXCLUDE (nooit repost/like)"},
}
...
SLOT_AUTHOR_FEED_LIMIT = int(os.getenv("SLOT_AUTHOR_FEED_LIMIT", "200"))
...
def parse_time_from_postview(post) -> Optional[datetime]:
    # Prefer indexedAt if available, else record.createdAt
...
def build_candidates_from_feed_items(
    ...
    for item in items:
        ...
        created = parse_time_from_postview(post)
        if not created:
            continue
...
def repost_and_like(
    ...
    # if already reposted and not refresh: skip
    if subject_uri in repost_records and not force_refresh:
        return False
...
def main():
    ...
    # Collect candidates (feeds + lists + hashtag) within cutoff
