...
def fetch_list_members(client: Client, list_uri: str, limit: int) -> List[Tuple[str, str]]:
    ...
    while True:
        page += 1
        if page % 5 == 1:
            log(f"👥 list fetch page {page} (members so far: {len(members)})")
...
def build_candidates_from_feed_items(items: List, cutoff: datetime,
                                     exclude_handles: Set[str], exclude_dids: Set[str],
                                     promo_force_refresh: bool) -> List[Dict]:
    cands: List[Dict] = []
    for item in items:
        ...
        cands.append({
            "uri": uri, "cid": cid, "created": created,
...
def build_candidates_from_postviews(posts: List, cutoff: datetime,
                                    exclude_handles: Set[str], exclude_dids: Set[str],
                                    promo_force_refresh: bool) -> List[Dict]:
    cands: List[Dict] = []
    for post in posts:
...
def force_unrepost_unlike_if_needed(client: Client, me: str, subject_uri: str,
                                   repost_records: Dict[str, str], like_records: Dict[str, str]):
...
def repost_and_like(client: Client, me: str, subject_uri: str, subject_cid: str,
                    repost_records: Dict[str, str], like_records: Dict[str, str],
    ...
    try:
        out = client.app.bsky.feed.repost.create(
            repo=me,
            record={"subject": {"uri": subject_uri, "cid": subject_cid},
...
def pick_random_post_from_actor_unlimited(client: Client, actor: str,
                                         exclude_handles: Set[str], exclude_dids: Set[str]) -> Optional[Tuple[str, str]]:
...
def main():
    ...
    for key, note, luri in excl_uris:
        log(f"🚫 Exclude lijst laden: {key} ({note})")
    ...
    log(f"Processing feeds: {len(feed_uris)}")
    for key, note, furi in feed_uris:
        log(f"📥 Feed verwerken: {key} ({note})")
        items = fetch_feed_items(client, furi, max_items=FEED_MAX_ITEMS)
        all_candidates.extend(build_candidates_from_feed_items(items, cutoff, exclude_handles, exclude_dids, promo_force_refresh=False))

    log(f"Processing lists: {len(list_uris)}")
    for key, note, luri in list_uris:
        log(f"📋 Lijst verwerken: {key} ({note})")
        members = fetch_list_members(client, luri, limit=max(1000, LIST_MEMBER_LIMIT))
        ...
        for (h, d) in members:
            ...
            all_candidates.extend(build_candidates_from_feed_items(author_items, cutoff, exclude_handles, exclude_dids, promo_force_refresh=False))

    log("Searching hashtag...")
    hashtag_posts = fetch_hashtag_posts(client, HASHTAG_MAX_ITEMS)
    all_candidates.extend(build_candid